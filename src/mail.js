...
mail.send = (from, recip, message, client) =>
  new Promise((f, r) => {
    let mailbox = storage.getItem(`mailbox-${mail.escape(recip)}`);
    ...
    if (!sent) {
      ...
      if (queued.length < 5) {
        ...
        f(
          'You have too many undelivered messages for this user.  Wait for them to be viewed before sending more.'
        );
      }
    }
  });
...
mail.startupDeliver = client => {
  ...
  storage.keys().forEach(key => {
    ...
    if (split[0] == 'mailbox') {
      ...
      if (mailbox && mailbox.new.length > 0) {
        //mailbox has new messages
        Object.keys(userlist).forEach(user => {
          if (
            mail.escape(user.toLowerCase()) == split[1].toLowerCase().replace(/ |-|'|"|`/gi, '_')
          ) {
